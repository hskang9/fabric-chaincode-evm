...
func TestInit(t *testing.T) {
	...
	stub := mocks.NewMockStub("evmscc", evmscc)
...
func TestDeployTwoContracts(t *testing.T) {
	evmscc := new(EvmChaincode)
	stub := mocks.NewMockStub("evmscc", evmscc)
	creator, err := marshalCreator("TestOrg", []byte(user0Cert))
	require.NoError(t, err)
	...
	// Install
	res := stub.MockInvoke("installtxid0", [][]byte{[]byte(account.ZeroAddress.String()), deployCode})
	assert.Equal(t, int32(shim.OK), res.Status, "expect OK, got: %s", res.Message)
	...
	// GetCode
	res = stub.MockInvoke("getcode", [][]byte{[]byte("getCode"), []byte(contract0Addr.String())})
	assert.Equal(t, int32(shim.OK), res.Status, "expect OK, got: %s", res.Message)
	...
	// Install
	res = stub.MockInvoke("installtxid1", [][]byte{[]byte(account.ZeroAddress.String()), deployCode})
	assert.Equal(t, int32(shim.OK), res.Status, "expect OK, got: %s", res.Message)

	contract1Addr, err := account.AddressFromHexString(string(res.Payload))
	assert.NoError(t, err)

	assert.NotEqual(t, contract0Addr, contract1Addr, "expect two different contract addresses")
}
