...
var _ = Describe("EndToEnd", func() {
	...
	BeforeEach(func() {
		...
		*/
		...
		runtimeCode = []byte("")
	})
	...
	It("executes a basic solo network with 2 orgs", func() {
		...
		By("deploying a SimpleStorage smart contract through the evmscc")
		...
		args := fmt.Sprintf(`{"Args":["%s","%s"]}`, account.ZeroAddress.String(), string(deployCode))
		adminRunner := adminPeer.InvokeChaincode("evmscc", w.Deployment.Channel, args, w.Deployment.Orderer)
		execute(adminRunner)
...
func compilePlugin() string {
	pluginFilePath := filepath.Join("testdata", "plugins", "evmscc", "evmscc.so")
	cmd := exec.Command(
		"go",
		append([]string{
			"build", "-buildmode=plugin", "-o", pluginFilePath,
			fmt.Sprintf("github.com/hyperledger/fabric-chaincode-evm/plugin"),
		})...,
	)
	cmd.Run()
